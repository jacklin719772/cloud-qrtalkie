...
export default function PlanManagement({ onNavigate }) {
  ...
  const getReviewStatus = (orderStatus) => {
    if (orderStatus === 'review_approved') return { label: '通過', style: STATUS_STYLES.active };
    if (orderStatus === 'review_rejected') return { label: '未通過', style: STATUS_STYLES.inactive };
    if (orderStatus === 'pending_review') return { label: '待審核', style: STATUS_STYLES.pending };
    return { label: '-', style: STATUS_STYLES.inactive };
  };

  const getBusinessOrderStatus = (orderStatus) => {
    if (orderStatus === 'review_approved') return { label: '已生效', style: STATUS_STYLES.active };
    if (orderStatus === 'review_rejected') return { label: '未生效', style: STATUS_STYLES.inactive };
    if (orderStatus === 'pending_review' || orderStatus === 'payment_submitted') return { label: '已提交', style: STATUS_STYLES.pending };
    return { label: '未提交', style: STATUS_STYLES.pending };
  };
  ...
  return (
    <>
      <style>{`
        ...
        #plan-management .plan-table .plan-action-cell { overflow: visible !important; }
        #plan-management .plan-table .plan-action-cell span { overflow: visible !important; max-width: none !important; }
              ...
              <tr style={{ background: '#1e293b' }}>
                <th style={{ width: '40px', padding: '12px 8px', borderBottom: '2px solid #2d3a4a', textAlign: 'center', position: 'sticky', top: 0, zIndex: 2, background: '#1e293b' }}>
                  ...
                  />
	                ...
	                <th style={{ width: '140px', padding: '12px 8px', fontSize: '12px', fontWeight: 600, color: '#e5e7eb', borderBottom: '2px solid #2d3a4a', textAlign: 'left', position: 'sticky', top: 0, zIndex: 2, background: '#1e293b', whiteSpace: 'nowrap' }}>套餐名稱</th>
	                <th style={{ width: '90px', padding: '12px 6px', fontSize: '12px', fontWeight: 600, color: '#e5e7eb', borderBottom: '2px solid #2d3a4a', textAlign: 'center', position: 'sticky', top: 0, zIndex: 2, background: '#1e293b', whiteSpace: 'nowrap' }}>訂單狀態</th>
	                <th style={{ width: '90px', padding: '12px 6px', fontSize: '12px', fontWeight: 600, color: '#e5e7eb', borderBottom: '2px solid #2d3a4a', textAlign: 'center', position: 'sticky', top: 0, zIndex: 2, background: '#1e293b', whiteSpace: 'nowrap' }}>審核狀態</th>
	                ...
	                <th style={{ width: '180px', padding: '12px 8px', fontSize: '12px', fontWeight: 600, color: '#e5e7eb', borderBottom: '2px solid #2d3a4a', textAlign: 'center', position: 'sticky', top: 0, right: 0, zIndex: 3, background: '#1e293b', whiteSpace: 'nowrap', borderLeft: '1px solid #374151' }}>操作</th>
              </tr>
            </thead>
            <tbody>
                {orders.map((order) => {
                  ...
                  <tr key={order.id}>
                    ...
                    <td style={{ position: 'sticky', right: 0, backgroundColor: '#111827', zIndex: 2, borderLeft: '1px solid #374151', width: '180px', padding: '12px 8px', borderBottom: '1px solid #1f2937', textAlign: 'center' }}>
        ...
        <div className="modal-overlay" onClick={() => { setDetailOrder(null); setShowCostDetails(false); }} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(15,23,42,0.6)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ width: '640px', backgroundColor: '#111827', borderRadius: '12px', display: 'flex', flexDirection: 'column', maxHeight: '85vh', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.1)' }}>
            <div className="modal-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 24px', borderBottom: '1px solid #1f2937' }}>
              ...
              <button onClick={() => { setDetailOrder(null); setShowCostDetails(false); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9ca3af', fontSize: '20px' }}>&times;</button>
            </div>
