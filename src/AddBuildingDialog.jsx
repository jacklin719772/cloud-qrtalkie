    ...
    <>
      ...
      <dialog ref={dialogRef} style={{ border: '0', borderRadius: '16px', padding: '0', maxWidth: '560px', width: '90vw', boxShadow: '0 20px 60px rgba(0,0,0,0.4)', background: '#111827', color: '#9ca3af' }}>
        <style>{`
          ...
          .add-building-body input, .add-building-body textarea, .add-building-body select {
        ...
        <form ref={formRef} method="dialog" onSubmit={handleSubmit} className="add-building-form">
          <div className="add-building-header">
            <h2 style={{ margin: 0, fontSize: '18px', fontWeight: 600, color: '#f3f4f6' }}>{isEdit ? '編輯樓宇' : '新增樓宇'}</h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
              {context.community?.name && (
                <div style={{ gridColumn: '1 / -1', fontSize: '13px', color: '#9ca3af', background: '#1a2332', padding: '8px 12px', borderRadius: '8px', border: '1px solid #1f2937' }}>
                  所屬社區：<strong style={{ color: '#f3f4f6' }}>{context.community.name}</strong>
                </div>
              )}

              <label style={{ gridColumn: '1 / -1' }}>
                <span style={{ display: 'block', fontSize: '13px', fontWeight: 500, color: '#9ca3af', marginBottom: '6px' }}>樓宇名稱 <b style={{ color: '#dc2626' }}>*</b></span>
                <input name="name" required style={{ width: '100%', height: '40px', padding: '0 12px', border: '1px solid #374151', borderRadius: '8px', fontSize: '13px', color: '#9ca3af', outline: 'none', boxSizing: 'border-box' }} placeholder="例如：A 座" />
              </label>
