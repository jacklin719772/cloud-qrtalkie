...
`}</style>
      ...
      {showPayments && createPortal((
        <div style={{ position: 'fixed', inset: 0, zIndex: 2147483647, background: 'rgba(15,23,42,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowPayments(false)}>
          <div style={{ background: '#fff', borderRadius: '16px', padding: '24px', maxWidth: '700px', width: '90%', maxHeight: '80vh', display: 'flex', flexDirection: 'column' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '16px', flexShrink: 0 }}>
              <h3 style={{ margin: 0, fontSize: '18px', fontWeight: 600 }}>付款記錄</h3>
              <button onClick={() => setShowPayments(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8', fontSize: '18px' }}>✕</button>
            </div>
            {paymentLoading ? <div style={{ textAlign: 'center', padding: '40px', color: '#94a3b8' }}>載入中...</div> : !paymentData ? <div style={{ textAlign: 'center', padding: '40px', color: '#94a3b8' }}>暫無數據</div> : (<>
              <div style={{ display: 'flex', gap: '12px', marginBottom: '16px', flexShrink: 0 }}>
                <span style={{ padding: '6px 14px', borderRadius: '999px', background: '#f8fafc', border: '1px solid #e2e8f0', fontSize: '12px' }}>記錄總數 <strong>{paymentData.totalCount}</strong></span>
                <span style={{ padding: '6px 14px', borderRadius: '999px', background: '#f0fdf4', border: '1px solid #86efac', fontSize: '12px', color: '#15803d' }}>付款總額 <strong>¥{paymentData.totalAmount}</strong></span>
              </div>
              <div style={{ flex: 1, overflow: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', minWidth: '550px' }}>
                  <thead><tr style={{ background: '#f8fafc' }}><th style={{ padding: '8px 12px', textAlign: 'left', fontWeight: 600, color: '#64748b', fontSize: '11px' }}>訂單編號</th><th style={{ padding: '8px 12px', textAlign: 'right', fontWeight: 600, color: '#64748b', fontSize: '11px' }}>金額</th><th style={{ padding: '8px 12px', textAlign: 'left', fontWeight: 600, color: '#64748b', fontSize: '11px' }}>付款方式</th><th style={{ padding: '8px 12px', textAlign: 'left', fontWeight: 600, color: '#64748b', fontSize: '11px' }}>付款日期</th><th style={{ padding: '8px 12px', textAlign: 'center', fontWeight: 600, color: '#64748b', fontSize: '11px' }}>憑證</th></tr></thead>
                  <tbody>{paymentData.payments.map((p, i) => <tr key={i} style={{ borderTop: '1px solid #f1f5f9' }}><td style={{ padding: '8px 12px' }}>{p.orderNo || '—'}</td><td style={{ padding: '8px 12px', textAlign: 'right', fontWeight: 500 }}>¥{Number(p.amount).toFixed(2)}</td><td style={{ padding: '8px 12px', color: '#64748b' }}>{p.paymentChannel || p.paymentMethod || '—'}</td><td style={{ padding: '8px 12px', color: '#64748b' }}>{p.paidAt || '—'}</td><td style={{ padding: '8px 12px', textAlign: 'center' }}>{p.proofFileUrl ? <a href={p.proofFileUrl.startsWith('/') ? '/api' + p.proofFileUrl : p.proofFileUrl} target="_blank" style={{ color: '#2563eb', fontSize: '12px', textDecoration: 'none' }}>查看</a> : '—'}</td></tr>)}</tbody>
                </table>
              </div>
            </>)}
          </div>
